...
class COCODataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.coco = COCO(os.path.join(self.data_root, 'annotations', self.json_file))
    ...
    def load_image_target(self, index):
        ...
        # load an image
        img_file = os.path.join(self.data_root, self.image_set,
                                '{:012}'.format(index) + '.jpg')
        image = cv2.imread(img_file)
        
        if self.json_file == 'instances_val5k.json' and image is None:
            img_file = os.path.join(self.data_root, 'train2017',
                                    '{:012}'.format(index) + '.jpg')
            image = cv2.imread(img_file)
        ...
        if is_train:
            ...
            for i in range(80):
                ...
                if heatmap.sum() > 0.:
                    ...
                    # class name
                    cls_name = coco_class_labels[coco_class_index[i]]
